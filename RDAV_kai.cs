	...
	List<IMyRemoteControl> FunPointer = new List<IMyRemoteControl>();
	...
	void Main(string argument)
            {
                ...
                IMyRemoteControl ActiveDirector = null;
	...
	void Init_RotorDirector()
			{
				...
				List<IMyRemoteControl> UsedPointer = new List<IMyRemoteControl>();
...
//                List<IMyRemoteControl> FunPointer = new List<IMyRemoteControl>();
				...
				List<IMyRemoteControl> DIRECTIONAL = new List<IMyRemoteControl>();
				GridTerminalSystem.GetBlocksOfType<IMyRemoteControl>(DIRECTIONAL);
...
/**/                NewDirector.Pointer = DIRECTIONAL.FindAll(x => x.CubeGrid == Key_Pointer.CubeGrid);
	...
	class RotorDirector
			{
				public IMyRemoteControl DIRECTOR;
				public IMyTerminalBlock AzimuthRotor;
				public IMyTerminalBlock ElevationRotor;
				public List<IMyRemoteControl> Pointer = new List<IMyRemoteControl>();
				public bool IsLeftRotor;
			}
	...
	void TurretControl(RotorTurret Turret, RotorDirector Director)
            {
				...
				Single TCURRENTAZ = ((float)57.2958 * TAZ.Angle);
                Single TCURRENTEL = ((float)57.2958 * TEL.Angle);
				if (TEL.CustomName.Contains("#LEFT#"))
					TCURRENTEL = -1 * TCURRENTEL;
				Single DCURRENTAZ = ((float)57.2958 * DAZ.Angle);
                Single DCURRENTEL = ((float)57.2958 * DEL.Angle);
				
				Single AZOVER = (DCURRENTAZ - TCURRENTAZ) ;
				
				if (DCURRENTAZ > 180)
				{
					DCURRENTAZ = DCURRENTAZ - 360;
				}
				
				else if (DCURRENTAZ < -360)
				{
					DCURRENTAZ = DCURRENTAZ + 360;
				}
